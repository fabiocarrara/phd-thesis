...
In fact, our results on a manually annotated benchmark for visual sentiment analysis %\cite{You2015:Robust}
show that the prediction accuracy of our trained CNN models is better than, or in the worst cases, in line with, state-of-the-art classifiers trained on sentiment-related data sets. %~\cite{Borth2013:Large-scale,Campos2016,Islam2016,Siersdorfer2010,You2015:Robust,Yuan2013:Sentribute}.
...
First approaches to infer affects from images were based on supervised or semi-supervised frameworks that map low-level features of images into human emotions \cite{Machajdik2010,Siersdorfer2010}. %Jia2012 %taglio ref
% Siersdorfer \etal \cite{Siersdorfer2010} firstly applied and evaluated automatic classification of sentiment using a large-scale collection of photos (crawled from Flickr using the SentiWordNet \cite{Esuli2006:sentiwordnet} dictionary as query terms).
...
\cite{Campos2017,Chen2014:DeepSentiBank,Islam2016,Rao2016,You2015:Robust}. %Xu2014, %%taglio
Most of these have in common the use of CNNs trained, or fine-tuned, on sentiment-related and labeled data. In \cite{Campos2017,Chen2014:DeepSentiBank,Islam2016,Jou2015:VisualAffect}, %,Xu2014 %Sun2016 %VGGNet \cite{simonyan2014very}
...
%%eliminate citazioni ad altri paper come Beacchi2015, Cha2016 Sun2016,
...
in numerous vision tasks \cite{Amato2017:parking,Amato2016:JOCCH,krizhevsky2012imagenet,razavian2014cnn,simonyan2014very}. %girshick2014rich %%taglio
...
% The architecture of \textit{HybridNet} %is the same of the \textit{BVLC Reference CaffeNet}~\cite{caffe2014}, which
% mimics the original AlexNet \cite{krizhevsky2012imagenet} with minor variations as described in~\cite{zhou2014}. %\cite{caffe2014}.
% It has $8$ weight layers: $5$ convolutional and $3$ fully-connected. The model {was} trained on $1,183$ categories ($205$ scene categories from Places205~\cite{zhou2014} and $978$ object categories from the train data of ImageNet~\cite{ImageNet}) with about $3.6$ million images.
% The \textit{VGG-19} is an improved version of the model used by the VGG team of the University of Oxford in the ILSVRC-2014 competition, as described in \cite{simonyan2014very}. The model {was} trained on $1,000$ categories of ImageNet~\cite{ImageNet} with about $1.5$ million images. It contains $19$ weight layers ($16$ convolutional + $3$ fully-connected).
...
We assessed the performance of our models using our \BTSA\, test set and the so-called Twitter Testing Dataset presented in \cite{You2015:Robust}. The latter contains a total of $1,269$ images having a positive or negative sentiment that have been manually labeled by five Amazon Mechanical Turk (AMT) workers. The images are partitioned into three subsets, namely ``Five agree", ``At least four agree", and ``At least three agree", on the basis of the labeling results from the five AMT workers. The details of this dataset are reported in Table~\ref{TwitterTestingDataset}. %The Twitter Testing Dataset
Twitter Testing Dataset was also used in \cite{Campos2017,Islam2016,Li2017,You2015:Robust}, allowing for a through comparison of our systems with the state-of-the-art. %Sun2016
...
\bottomrule
\end{tabularx}
\caption{Twitter Testing Dataset \cite{You2015:Robust}.} \label{TwitterTestingDataset}
\end{table}
...
\begin{table}
    ...
    CNN~\cite{You2015:Robust}   & 0.722 & 0.686 & 0.667 & -     \\
    PCNN~\cite{You2015:Robust}  & 0.747 & 0.714 & 0.687 & -     \\
...
\end{table}
As baseline references, we report the results obtained by a random classifier and by the CNN and PCNN models \cite{You2015:Robust}.
The results show that the models trained with our cross-media approach effectively classify the images of Twitter Testing Dataset which were manually annotated. In particular, our best model ({\ourFtVGG} FT-A) correctly classifies $78.5\%$ of the \textit{five agree} testing images, outperforming similar models trained on high-quality sentiment-related hand-labeled data~\cite{You2015:Robust}.
...
Many papers on visual sentiment analysis report the 5-fold cross-validation accuracy on the Twitter Testing Dataset~\cite{Campos2017,Islam2016,Li2017,You2015:Robust}, in which the prediction of each fold is computed with a model fine-tuned on the other four folds. In order to compare to those approaches, we also tested our models in this setting, and we reported the 5-fold accuracy obtained in Table \ref{tab:Twitter882_5fold}.
...
% In order to compare the performance of our model to other state-of-the-art methods, we also performed a 5-fold cross-validation on the testing subsets (as also done in \cite{Campos2016,Islam2016,Li2017,You2015:Robust}), in which we compute the prediction of each fold with a model fine-tuned again on the other four folds. Results for 5-fold cross-validation are reported in Table \ref{tab:Twitter882_5fold}.
\begin{table}
\small
\newcolumntype{R}{>{\raggedleft\arraybackslash}X}
...
\midrule
GCH~\cite{Siersdorfer2010}              {\footnotesize (from~\cite{You2015:Robust})} \tnote{$\ast$}  &   -   &   0.684   &   0.665   &   0.66    \\
SentiBank~\cite{Borth2013:Large-scale}  {\footnotesize (from~\cite{You2015:Robust})} \tnote{$\circ$} &   -   &   0.709   &   0.675   &   0.662   \\
LCH~\cite{Siersdorfer2010}              {\footnotesize (from~\cite{You2015:Robust})} \tnote{$\ast$}  &   -   &   0.710   &   0.671   &   0.664   \\
GCH+ BoW~\cite{Siersdorfer2010}         {\footnotesize (from~\cite{You2015:Robust})} \tnote{$\ast$}  &   -   &   0.710   &   0.685   &   0.665   \\
LCH+ BoW ~\cite{Siersdorfer2010}        {\footnotesize (from~\cite{You2015:Robust})} \tnote{$\ast$}  &   -   &   0.717   &   0.697   &   0.664   \\
Sentribute~\cite{Yuan2013:Sentribute}   {\footnotesize (from~\cite{You2015:Robust})} \tnote{$\circ$} &   -   &   0.738   &   0.709   &   0.696   \\

CNN~\cite{You2015:Robust} \tnote{$\bullet$}   &   Flickr (VSO)    &   0.783   &   0.755   &   0.715   \\
AlexNet {\footnotesize (from~\cite{Campos2017})} \tnote{$\bullet$}   &  \gls{ilsvrc}12   &   0.817   &   0.782   &   0.739   \\
PlaceCNN~\cite{zhou2014}                {\footnotesize (from~\cite{Campos2017})} \tnote{$\bullet$}   &   Places205    &   0.830   &   -   &   -   \\
GoogleNet~\cite{Szegedy2015:GooleNet}   {\footnotesize (from~\cite{Islam2016})}  \tnote{$\bullet$}   &   \gls{ilsvrc}12   &   0.861   &   0.807   &   0.787   \\
...
PCNN~\cite{You2015:Robust} \tnote{$\bullet$}   &   Flickr (VSO) + \textit{ft} Flickr (VSO)  &   0.773   &   0.759   &   0.723   \\
