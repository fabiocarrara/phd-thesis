...
We propose %the preliminary results on learning
...
\includegraphics[width=\linewidth]{figures/visreg-t2v-arch}
...
We compute $r_i = \text{ROUGE}_L ( \t, C_i ) $, where $\t$ is the query caption, and $C_i$ are the five captions associated to the retrieved image at rank $i$.
...
An interesting aspect that proved to be effective in our experiments is the use of a different $\t'$ as a constraint for the hidden representation.
