...
Inputs and outputs of convolutional layers are also called \emph{feature maps}, since each value in the two-dimensional map usually is interpreted as the presence or absence of a feature a particular filter has learnt to identify.
\ref{eq:back:cross-correlation} defines the cross-correlation operation for inputs and outputs having both a single feature maps.
Images instead are represented as 3-D tensors having $C$ channels (e.g. $C=3$ for RGB data, $C=1$ for grayscale); thus the definition of the cross-correlation is extended in such way that filters span the depth of the input tensor.
