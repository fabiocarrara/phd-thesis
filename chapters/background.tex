...
In this chapter, we provide the reader with the basic concepts about \acrlong{dl} and \acrfullpl{cnn}.
...
In \ref{sec:back:image-retrieval}, we describe the main aspects of \gls{cbir} based on image representations extracted from deep neural networks, and we discuss some state-of-the-art methodologies to build effective descriptions of images and to index them in large-scale scenarios efficiently.
...
% TODO [?] expand on representation learning, what is and cites
\acrfull{dl} defines a subfield of Artificial Intelligence, specifically of \gls{ml} and Representation Learning, in which a \emph{hierarchy of data representations} (or \emph{features}) is learned from data for solving a particular task~\cite{bengio2007scaling,goodfellow2016deep}.
...
% TODO add references to similarities to the mammalian brain (in addition to perceptron?)
Inspired by nature, \acrlong{dl} models are often implemented as \acrfullpl{dnn} --- a computational model comprised of multiple layers of processing units that mimics the structure and the interconnection of neurons in the mammalian brain~\cite{rosenblatt1958perceptron}.
Recent years have witnessed a rapid rise in the use of \gls{dl} approaches to solve complex tasks, reaching even super-human performance in perceptual~\cite{he2015delving}, control~\cite{mnih2015human}, and planning~\cite{silver2016mastering} activities. % TODO [?] add reasoning
Interestingly, the representations learned with \gls{dl} methodologies resemble the structures of signals in the neocortex build by our brain to implement intelligent behaviors, suggesting strong parallelism between the two domains~\cite{cadieu2014deep,kubilius2016deep}. % TODO [?] dire meglio

\acrlongpl{dnn} are organized as a sequence (or more generally a graph) of non-linear parametric transformations, known as \emph{layers}, that acts like features extractors;
...
Given a training set of input-target couples $\X = \{ (\x_i, \y_i^\star),\; i = 1, \dots, N \}$, the quality of a particular configuration of parameters is quantitatively defined by a \emph{loss function} $\L\(\X; \Theta\)$ that measures how much the model predictions $\y$ differ from the targets $\y^\star$ provided by $\X$.
...
\begin{figure}
    ...
    The \acrfull{relu} on the left, the sigmoid $\sigma$ in the middle, and hyperbolic tangent on the right.}
...
Formally, given an input comprised of $n$ features $\x \in \R^n$, the output  $\y \in \R^m$ of the layer is obtained as
%
...
The dimensionalities of the input $n$ and the output $m$ are referred to as respectively the number of \emph{input features} and \emph{output features}.

\begin{figure}
    \centering
    \includegraphics[width=0.8\linewidth]{figures/neuron}
    \caption{The parallelism between a real neuron and the artificial one used in \glspl{mlp}.}
...
this is modeled in the perceptron defining its output as the activation function $\a(\cdot)$ applied to the inner product between input and weights
%
\begin{equation}
...
In \ref{ch:cross-media,ch:text2vis}, we will adopt this kind of models to extract information from texts accompanying images, and thus we provide the reader with the basic concepts and models involved.

\begin{figure}
    \centering
    \includegraphics[width=.9\linewidth]{figures/rnn}
    \caption{Basic architecture of a \acrlong{rnn} (left).
    On the right, its unrolled version for a sequence of length 5.}
    \label{fig:back:rnn}
\end{figure}
...
In its simplest form, the recurrent cell $f$ can be implemented using a fully-connected layer that operates on the concatenation of the current state and input
%
\begin{equation}
    \h_t = \W [ \x_t | \h_{t-1} ] + \b \,,
\end{equation}
...
\begin{figure}
    ...
    \caption{The unrolled version of a Bidirectional \gls{rnn} for a sequence of length 5.}
...
Regularization has gained importance in the \gls{dl} field due to the vast amount of parameters models are usually comprised of, thus increasing the risk of overfitting on the train data.
...
\end{equation}
%
Classification models are often designed to output an $N$-dimensional vector $\z$ that is mapped to a categorical distribution via the \emph{softmax} function
%
    ...
    \begin{tabularx}{\linewidth}{C@{\hskip 3em}C}
        \multirow{2}{*}[4em]{
            \begin{subfigure}{\linewidth}
                ...
                \caption{A ranking loss tends to pull similar elements (black dots) near to an anchor $\x_a$ (i.e., the query point) and to push dissimilar ones (gray dots) away from it.
...
The direction given by $\nabla\L\(\X; \Theta\)$ is the one having maximum steepness of the loss surface in the parameter space, that is, the one that maximizes the loss change locally.
...
The formulations of the loss function $\L$ and intermediate layers $f_i$ are chosen to be differentiable to ensure that also the entire model --- being a composition of differentiable functions --- is, in turn, differentiable, and that back-propagation can be applied to compute gradients efficiently.
...
The employment of this technique usually increases the number of training iterations needed to a model to converge, because only the subset of weights of the survived neuron is updated in an optimization step.
...
% Examples (simple classif., sentiment analysis, etc.), classifying images is super cool bla bla
...
We refer to \emph{single-label} $L$-way classification if only one label has to be picked among the $L$ available, referring to the particular case in which $L=2$ as \emph{binary} classification.
...
Notice that the same metrics can be applied to cases with $L > 2$ by reformulating them as $L$ binary problems, where one class is the positive class, and the other $(L-1)$ are grouped in the negative class;
...
Traditional approaches to image classification required to manually design at least partially the classifier $f(\cdot)$.
...
However, manually engineered solutions are feasible for particular problems --- such as image stitching~\cite{brown2007automatic} and 3d reconstruction~\cite{schonberger2016structure} --- where the conditions are well known and modeled.
...
\begin{table}
    \begin{tabularx}{\linewidth}{p{4.5cm}X>{\centering}c}
        ...
        \bottomrule
    \end{tabularx}
    \caption{\gls{ilsvrc}'12 leaderboard reporting the accuracy (as top-5 error percentage) on the image classification task obtained by the participating teams.
    Results taken from~\cite{russakovsky2015imagenet}.}
    \label{tab:ilsvrc12-results}
\end{table}

On the other hand, \acrlong{dl} approaches rely on a large amount of labeled data to automatically discover which features and combination of them are relevant for the task.
...
This amazing results provided a springboard for the research community for a new way to to do image classification and in general to learn high-quality features for images.
...
\paragraph{ZFNet}
Despite \glspl{cnn} being remarkably good in solving vision task,
...
This improved architectures won the \gls{ilsvrc}'13 classification task with an 11.7\% top-5 error.

% TODO [?,FIGURE] add deconvolution visualization from zeiler2014visualizing, big figure or fig 6?
%\begin{figure}
%\centering
...
\paragraph{OverFeat}
\citet{sermanet2013overfeat} proposed \emph{OverFeat}, a modification of the AlexNet architecture optimized for object localization and detection in images, where the model is asked to predict also the bounding boxes of the object of interest.
...
\citet{szegedy2015going} proposed to improve the efficiency of the network employing a new architectural block called \emph{Inception module}.
...
this kind of architecture is called \gls{resnet}.
...
At the time of writing, one of the most recent works improving \glspl{dcnn} for image classification and recognition is \glspl{senet}~\cite{hu2017squeeze}.
...
We indicate as \acrfull{cbir} the task of retrieving from a (potentially large) collection of images the ones relevant to a query by looking only to their unstructured content, i.e., without exploiting additional information associated with images such as textual annotations or structured metadata as traditional approaches would.
\gls{cbir} systems are then valuable in the case image metadata --- e.g., annotations, surrounding text in documents, descriptions --- is scarce, misleading, or absent, and traditional approaches would fail.
...
\begin{figure}
    ...
    \caption{General structure of a \acrfull{cbir} system.
    Round rectangles indicate inputs and outputs of the system.}
    \label{fig:back:cbir}
\end{figure}
...
starting from raw pixels, the content of an image is encoded in a compact numerical representation, commonly called \emph{feature vector}, on which we rely to assess the similarity between images.
...
% query formulation
In the online phase, the user first formulates a query to inject in the system its concept of the content to be retrieved.
...
However, more complex paradigms are also possible, such as query by tag, sketch, and color map, to name a few.
...
For other paradigms, this step may be arbitrarily complex depending on the query modality, but usually enables advanced search possibilities, such as cross-media image retrieval that will be discussed in \ref{ch:text2vis};
...
%that outperformed handcrafted features
...
The image retrieval problem is not so different from classical textual information retrieval problem, and thus it borrows from it much of the terminology, formulations, and evaluation metrics.
...
This function permits to assign a similarity score (or a distance) to each element of our search collection and to sort them based on their relevance to the query.
...
The \emph{\acrfull{dcg}}~\cite{jarvelin2002cumulated} is conventionally used when multiple levels of relevancy are available for samples in the ground-truth.
It aims to penalize highly relevant samples in the bottom part of the results;
to do so,  a logarithmic weighting scheme is usually adopted to assign a decreasing importance to samples in the lower part of the search results.
...
\sloppy In the case rankings can have ties, the \gls{tdcg}~\cite{mcsherry2008computing} is used, defined as
%
\begin{equation}
...
The \gls{tdcg} is equivalent to \gls{dcg} in the case there are no ties in the results.

% NDCG is divided by a normalization factor to ensure the metric is in the $[0,1]$ range.

% R@K
\paragraph{\acrlong{r@k}}
The \acrfull{r@k} is the recall metric when considering only the top-K results of the ground-truth as relevant.
It is conventionally used to compare two rankings in the absence of an explicit relevancy labeling of the ground-truth.
...
These metrics are particularly useful when evaluating rankings in which there is only one object in the database relevant to the query that needs to be retrieved.
Cross-modal retrieval with coupled datasets is an example in which this condition applies: using as query one modality of a sample, a common approach is to evaluate how well the system can retrieve the same sample in the different modality.
...
It is a subset of roughly 1.3 million images taken from the larger ImageNet\footnote{\url{http://www.image-net.org}} database --- a big collection of images labeled and organized following the hierarchical categorization of WordNet.
Images are assigned a single label chosen from the 1,000 classes selected for the competition, spanning general objects, animals, and many others.
...
Due to its relevancy in large-scale image classification, it is extensively used, and many models pre-trained on this dataset are publicly available.
We adopt models pre-trained on \gls{ilsvrc} in \ref{ch:adversarial,ch:text2vis}.

\paragraph{Places205~\cite{zhou2014learning}}
The Places205 dataset is composed of roughly 2.5 million images collected from image search engines labeled with 205 semantic scene categories and attributes representing visual environments all over the world.
It is a subset of the bigger Places dataset~\cite{zhou2016places}.
We employ models pre-trained on \gls{ilsvrc} and Places205, also called HybridNets, in \ref{ch:cross-media}, where we fine-tune them for the specific task of visual sentiment analysis.
This combination of training sets leads to models with a general representation capability of generic objects and scenes.

\paragraph{PKLot~\cite{de2015pklot}}
The PKLot dataset is a car parking occupancy detection dataset comprised of 12,417 images of 3 parking lots --- named UFPR04, UFPR05, and PUC --- took in different days and weather conditions.
...
Each segmented image is labeled with the vacancy status of the slot, which can be either occupied or vacant.
...
It is comprised of roughly 150,000 images of parking spaces taken from nine smart cameras with different viewpoints, in different days with different weather and light conditions, and includes occlusion and shadow situations that make the occupancy detection task challenging.
We employ both the CNRPark-EXT and PKLot datasets in \ref{ch:miniaturization} to test the generalization performance of miniaturized \glspl{cnn} for embedded devices in the context of decentralized parking lot occupancy detection.
More details about the CNRPark-EXT dataset will be provided in \ref{ch:miniaturization}.

\paragraph{Twitter Testing Dataset~\cite{you2015robust}}
The Twitter Testing Dataset is a dataset for image sentiment analysis composed by 1,269 images labeled with crowd-sourcing (Amazon Mechanical Turk) following a binary positive/negative sentiment polarity scheme.
Each image has been annotated as positive or negative by five workers, and three different subsets have been created, respectively comprised of images where five workers agree on the sentiment of the image (5-agree, 882 images), where at least four workers agree (4-agree, 1,116 images), and where at least three workers agree (3-agree, whole set of images).
...
The INRIA Holidays image set is an image retrieval dataset comprised of 1,491 high-resolution holidays photos containing various scenes types.
...
we refer to this variant as \emph{Holidays-MIRFlickr1M}.
...
The set is composed of photos of 11 famous buildings plus distractor images collected from Flickr.
...
For each query, images of the search set are assigned with one of the following four labels:
\begin{itemize}
    ...
    \item \emph{junk} --- if less than 25\% of the building is visible, or there is a very high level of occlusion or distortion;
...
We employ both \emph{Holidays-MIRFlickr1M} and \emph{Oxford-Flickr100k} in \ref{ch:surrogate} to evaluate the large-scale image retrieval performance.
...
This dataset is also extensively adopted also in cross-modal retrieval due to its large scale and the presence of captions, and we will use it to compare to existing cross-modal retrieval approaches in \ref{ch:text2vis}.

\subsection{Others}

\paragraph{NIPS Adversarial Attacks and Defences Competition DEV set~\cite{kurakin2018adversarial}}
The NIPS Adversarial Attacks and Defences Competition DEV set is comprised of 1,000 images that do not belong to the \gls{ilsvrc}'12 subsets but share the same label space, i.e., are labeled with one of the same 1,000 labels used in \gls{ilsvrc}.
...
In particular, the set is devoted to generating adversarial examples for image classifiers, i.e., maliciously manipulated images the lead a classifier to misbehave.
